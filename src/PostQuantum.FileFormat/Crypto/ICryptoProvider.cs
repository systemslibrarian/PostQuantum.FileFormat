...
public interface ICryptoProvider
{
    ...
    (byte[] sk, byte[] pk) MlKem768GenerateKeyPair();
    (byte[] sharedSecret, byte[] ciphertext) MlKem768Encapsulate(ReadOnlySpan<byte> peerPk);
    byte[] MlKem768Decapsulate(ReadOnlySpan<byte> sk, ReadOnlySpan<byte> ciphertext);
