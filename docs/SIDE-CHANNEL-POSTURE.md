...
|---|---|---|---|
| [`HybridKem.cs`](../src/PostQuantum.FileFormat/Crypto/HybridKem.cs) `Encapsulate` / `Decapsulate` | Concatenate X25519 + ML-KEM-1024 shared secrets, derive KEK via HKDF. | Both halves always run. Intermediate shared secrets and the X25519 ephemeral private key are zeroed via `SecureZero.Clear` before return. | No additional leak. |
