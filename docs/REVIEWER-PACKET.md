...
## NIST ACVP cross-check (ML-KEM-768 + ML-DSA-87)

PQF's underlying primitives are validated against NIST's official
ACVP-Server test vectors, not just against each other. The KAT class
[`NistAcvpKatTests`](../tests/PostQuantum.FileFormat.Tests/Crypto/NistAcvpKatTests.cs)
loads a curated subset of vectors from
`github.com/usnistgov/ACVP-Server` (with provenance recorded in the
JSON resource itself) and asserts:

- **ML-KEM-768 decap** — 10 NIST `tgId=5` decapsulation vectors:
  given the published `(dk, c)`, PQF's `BclCryptoProvider.MlKem768Decapsulate`
  must produce the byte-for-byte expected `k`.
- **ML-DSA-87 sigVer (external, pure)** — 8 NIST `tgId=5` verify
  vectors (3 positive + 5 negative): the BCL `MLDsa.VerifyData(message,
  signature, context)` call must return the NIST-expected
  `testPassed` value, across mixed contexts and a positive/negative
  mix. An additional smoke test routes the empty-context vectors
  through PQF's production `BclCryptoProvider.MlDsa87Verify` wrapper
  to prove the wrapper hasn't drifted from the BCL call shape.

This closes the question "are you using `System.Security.Cryptography.MLKem`
/ `MLDsa` correctly?" with NIST-side evidence rather than implementation
intent.

## Supply-chain guarantees on the published artifact

`0.6.0-preview.2` and every subsequent release are produced by
[`.github/workflows/release.yml`](../.github/workflows/release.yml),
which generates **four** verifiable artifacts attached to the GitHub
release:

| Artifact | What it is | Verify with |
|---|---|---|
| `.nupkg` | The NuGet package itself. Reproducible (Deterministic=true, ContinuousIntegrationBuild, PathMap). | `dotnet nuget verify` + the SHA-256 from the release notes. |
| `.nupkg.sig` + `.nupkg.pem` | **Sigstore cosign keyless signature** — anchored in the public Rekor transparency log at `rekor.sigstore.dev`. No long-lived signing key exists; the certificate is short-lived OIDC-derived from the GitHub Actions workflow identity. | `cosign verify-blob --signature <sig> --certificate <pem> --certificate-identity-regexp 'https://github.com/systemslibrarian/PostQuantum.FileFormat/.+' --certificate-oidc-issuer https://token.actions.githubusercontent.com <nupkg>` |
| `pqf-cli.cdx.json` | CycloneDX **SBOM** listing every transitive dependency, license, and component hash. | Standard CycloneDX tooling. |
| SLSA provenance attestation | **SLSA Level 3** build provenance generated by the official `slsa-github-generator` workflow. Names the source commit, workflow file, runner, and inputs that produced the nupkg + SBOM. | `slsa-verifier` or `gh attestation verify`. |

The combination — reproducible build + cosign + Rekor + SBOM + SLSA L3 —
means a downstream consumer can verify, **without trusting the
maintainer's machine**, that the bytes they downloaded came from
this repository at a specific commit via the published CI workflow.
...
These require action outside the repository and are the remaining
distance to a "10/10" review artifact:

1. **At least one external cryptographic review** of the X-Wing
   integration (the combiner itself is externally proven; what's not
   externally reviewed is PQF's overall assembly: per-recipient AEAD
   wrap, chunked AEAD with `is_final` AAD binding, footer
   reconciliation, hybrid signature construction with domain
   separation). This is the single highest-value remaining step. See
   the [Seeking external cryptographic review](#seeking-external-cryptographic-review--call-to-action)
   section at the top of this packet for the four specific scoped
   questions.

### Recently closed (no longer gaps)
...
3. ~~NIST ACVP cross-check for the underlying primitives.~~ **Closed
   in this commit.** `NistAcvpKatTests` validates ML-KEM-768 decap
   (10 vectors) and ML-DSA-87 sigVer (8 vectors, 3+5 mixed) against
   `github.com/usnistgov/ACVP-Server` published vectors. The "are you
   using BCL MLKem / MLDsa correctly?" question is now answered with
   NIST-side evidence.
4. ~~Signed release artifacts.~~ **Closed in this commit.** Every
   release produced by `release.yml` is signed with Sigstore cosign
   keyless OIDC (anchored in the public Rekor transparency log) and
   carries a SLSA Level 3 build provenance attestation. See the
   "Supply-chain guarantees on the published artifact" section above
   for the consumer-side `cosign verify-blob` command.
