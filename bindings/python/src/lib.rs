...
const SPEC_VERSION: &str = "v1 draft 0.4.0 (X-Wing + ML-KEM-768)";
...
fn recipient_to_dict<'py>(
    ...
    d.set_item("classical_epk", PyBytes::new_bound(py, &recipient.classical_epk))?;
    d.set_item("pqc_ct", PyBytes::new_bound(py, &recipient.pqc_ct))?;
    d.set_item("wrapped_dek", PyBytes::new_bound(py, &recipient.wrapped_dek))?;
    d.set_item(
        "wrapped_dek_nonce",
        PyBytes::new_bound(py, &recipient.wrapped_dek_nonce),
    )?;
    Ok(d)
}
...
#[pyfunction]
fn parse_header<'py>(py: Python<'py>, file_bytes: &[u8]) -> PyResult<Bound<'py, PyDict>> {
    let parsed = rust_parse(file_bytes).map_err(map_err)?;
    let h = &parsed.header;
    let out = PyDict::new_bound(py);

    let alg = PyDict::new_bound(py);
    ...
    out.set_item("file_id", PyBytes::new_bound(py, &h.file_id))?;
    ...
    if let Some(signer) = &h.signer {
        ...
        s.set_item("classical_pub", PyBytes::new_bound(py, &signer.ed25519_pub))?;
        s.set_item("pqc_pub", PyBytes::new_bound(py, &signer.mldsa87_pub))?;
...
#[pyfunction]
fn decrypt<'py>(py: Python<'py>, file_bytes: &[u8], identity: &Identity) -> PyResult<Bound<'py, PyBytes>> {
    let parsed = rust_parse(file_bytes).map_err(map_err)?;
...
impl Identity {
    ...
    /// uses: id name, public-key blob (1217 bytes:
    /// 0x01 || x25519_pub(32) || ml_kem_768_pub(1184)), X25519 secret,
    /// ML-KEM-768 secret.
    #[staticmethod]
    fn from_manifest(
        id: &str,
...
#[pymodule]
fn pqf(m: &Bound<'_, PyModule>) -> PyResult<()> {
